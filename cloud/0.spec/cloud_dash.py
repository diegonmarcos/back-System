...
def export_js(output_path: str = None) -> str:
    ...
    Creates a JS file with CONFIG constant that the web dashboard
    can use in 'Local' mode without API calls.

    Args:
        output_path: Output file path (default: cloud_dash_data.js in spec dir)
    ...
    # Default output to spec directory and frontend
    if not output_path:
    ...
    # Build the JS content (using CONFIG to match HTML expectations)
    js_content = f"""// Auto-generated by cloud_dash.py
// Generated: {timestamp}
const CONFIG = {json_mod.dumps(config, indent=2)};
"""
